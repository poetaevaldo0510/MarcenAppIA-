      ...
      <div className="flex-1 flex flex-col relative h-[100dvh] bg-white lg:rounded-l-[4rem] shadow-2xl overflow-hidden">
        ...
        <footer className="absolute bottom-0 left-0 right-0 p-4 sm:p-8 bg-white/80 backdrop-blur-3xl border-t border-slate-100 z-[150] pb-safe">
          <div className="max-w-5xl mx-auto flex gap-4 items-end">
            <button onClick={() => setIsToolsOpen(!isToolsOpen)} className={`w-16 h-16 shrink-0 flex items-center justify-center rounded-[2rem] shadow-2xl transition-all ${isToolsOpen ? "bg-red-500 text-white rotate-45" : "bg-zinc-900 text-amber-500"}`}><Plus size={28} /></button>
            <div className={`flex-1 rounded-[2rem] flex flex-col border transition-all duration-500 shadow-2xl overflow-hidden ${isListening ? 'bg-amber-50 border-amber-400' : 'bg-white border-slate-200'}`}>
              ...
              <div className="flex items-center px-6">
                <button onClick={() => galInputRef.current?.click()} className="p-4 text-slate-400 hover:text-amber-600"><LucideImage size={26}/></button>
                <input 
                ...
                />
                <button onMouseDown={startRecording} onMouseUp={stopRecording} onTouchStart={startRecording} onTouchEnd={stopRecording} className={`p-4 transition-all ${isListening ? "text-red-500 scale-125 animate-pulse" : "text-slate-400 hover:text-amber-600"}`}><Mic size={30}/></button>
              </div>
            </div>
            <button onClick={() => handlePipeline(input, preview)} disabled={store.loadingAI || (!input.trim() && !preview)} className="w-16 h-16 shrink-0 flex items-center justify-center rounded-[2rem] shadow-2xl bg-zinc-900 text-amber-500 active:scale-90 disabled:opacity-30">
        ...
        {isToolsOpen && (
          <div className="fixed inset-0 z-[200]">
            ...
            <div className="absolute bottom-40 left-8 sm:left-12 w-80 bg-[#09090b] border border-white/10 rounded-[3rem] shadow-2xl p-5 flex flex-col gap-2">
              {[
                { id: 'BILLING', label: 'Créditos Hub', icon: Wallet, color: 'bg-amber-600' },
                { id: 'BENTO', label: 'Corte CNC Pro', icon: Scissors, color: 'bg-blue-600' },
                { id: 'ESTELA', label: 'Orçamentos Master', icon: DollarSign, color: 'bg-emerald-600' },
                { id: 'ADMIN', label: 'Núcleo Técnico', icon: Settings2, color: 'bg-slate-700' },
              ].map(tool => (
                <button key={tool.id} onClick={() => { store.setModal(tool.id); setIsToolsOpen(false); }} className="w-full flex items-center gap-5 p-5 hover:bg-white/5 rounded-[2.2rem] text-white transition-all group">
